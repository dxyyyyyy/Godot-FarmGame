...
# Recalculate the current day, hour, and minute based on the initial time
func recalculate_time() -> void:
	# Calculate the total minutes that have passed since the start of the game
...
# Emit signals if the minute or day has changed
	if current_minute != minute:
		current_minute = minute
		emit_signal("time_tick", day, hour, minute)
		
	if current_day != day:
		current_day = day
		emit_signal("time_tick_day", day)
